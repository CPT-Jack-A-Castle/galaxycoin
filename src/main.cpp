...
unsigned int nStakeMinAge = 60 * 60 * 24 * 5;	// 5 days as minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 15;	// 15 days as stake age of full weight
...
const string strMessageMagic = "Galaxycoin Signed Message:\n";
...
// galaxycoin: continuous target adjustment, based on last nTargetInterval blocks
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nTargetInterval != 0)
    {
        ...
        if (fTestNet)
        {
            ...
            else
            {
                ...
                while (pindex->pprev && pindex->nHeight % nTargetInterval != 0 && pindex->nBits == nProofOfWorkLimit)
                    pindex = pindex->pprev;
                return pindex->nBits;
            }
        }
    ...
    // Galaxycoin: This fixes an issue where a 51% attack can change difficulty at will.
    // Go back the full period unless it's the first retarget after genesis. Code courtesy of Art Forz
    int blockstogoback = nTargetInterval-1;
    if ((pindexLast->nHeight+1) != nTargetInterval)
        blockstogoback = nTargetInterval;
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlock *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
        ...
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                ...
                EventHorizonDeviation                        = 1 + (0.7084 * pow((double(PastBlocksMass)/double(30)), -1.228));
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlock *pblock)
{
        ...
        int64                       PastSecondsMin                               = TimeDaySeconds * 0.010;
        int64                       PastSecondsMax                               = TimeDaySeconds * 0.031;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 255145) { DiffMode = 2; }
        }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Galaxycoin
    bool fStrictPayToScriptHash = true; // Always active in Galaxycoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Galaxycoin: check proof-of-stake block signature
    if (IsProofOfStake())
    {
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Galaxycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMinAge = 2 * 60;							// test net min age is 2 mins
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "7/7/2013: Two people were dead and 182 others were taken to hospitals after Saturday morning's crash landing of an Asiana Airlines Boeing 777 at San Francisco International Airport.";
        CTransaction txNew;
        txNew.nTime = 1372902718;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1372902718;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 7875379 : 100;

        //// debug print
        printf("block.GetHash() = %s\n", block.GetHash().ToString().c_str());
        printf("block.hashMerkleRoot = %s\n", block.hashMerkleRoot.ToString().c_str());
		block.print();
		assert(block.hashMerkleRoot == uint256("0xfbbb423408ec6ccf1e5853dd53daa4cb78da9d1deb60ded1a9d03efdb5ac863d"));
...
string GetWarnings(string strFor)
{
    ...
    // galaxycoin: should not enter safe mode for longer invalid chain
    // galaxycoin: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 90) && !fTestNet && !IsInitialBlockDownload()) // 90 days
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old. Download a newer client version.";
    }
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc0, 0xe9, 0xdb }; 
